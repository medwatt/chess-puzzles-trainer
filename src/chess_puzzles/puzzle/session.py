...
from chess_puzzles.puzzle.tree import MoveTree, MistakeLine, TreeNode
...
class PuzzleSession:
    ...
    mistakes: int = 0
    aids_used: int = 0
    last_mistake_line: MistakeLine | None = field(init=False, default=None)
    ...
    def avoided_mistakes(self) -> list[tuple[str, MistakeLine]]:
        """Marked mistakes the user walked past: mistake-marked siblings at each of the
        user's decision points along the line played so far, as (FEN of the
        decision point, punishing line) pairs. Used after solving to offer the
        lesson the user never triggered."""
        if self._tree is None:
            return []
        avoided: list[tuple[str, MistakeLine]] = []
            ...
            if board.turn == self.player_color:
                for child in node.children:
                    if child.is_mistake and child.move != move:
                        avoided.append((board.fen(), MoveTree.mistake_line_of(child)))
    ...
    def _classify_deviation(self, move: chess.Move) -> MoveResult:
        ...
        if node.is_mistake:
            self.mistakes += 1
            self.last_mistake_line = MoveTree.mistake_line_of(node)
